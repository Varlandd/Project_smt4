...
class UserDashboardController extends Controller
{
    ...
    /**
     * Rekomendasi Finansial — page with form to calculate financial capability.
     */
    public function rekomendasiFinansial()
    {
        return view('pages.rekomendasi-finansial');
    }

    /**
     * Hitung kemampuan finansial dan cari rumah sesuai budget + ML clustering.
     * Rumus: budget = 3 × (pendapatan bersih × 12)
     * Juga memanfaatkan ML Service KNN untuk cluster matching.
     */
    public function hitungFinansial(Request $request)
    {
        ...
        if ($pendapatanBersih <= 0) {
            return response()->json([
                'status'  => 'error',
                'message' => 'Pendapatan bersih harus lebih dari 0. Pastikan pendapatan lebih besar dari pengeluaran.',
            ], 422);
        }
        ...
        try {
            ...
            // ML Service offline, fallback to query-only mode
            $mlStatus = 'offline';
        }
        ...
        // Tandai favorit
        $rumahs->transform(function ($item) use ($userId) {
            $item->is_favorit = $this->isFavorited($item, $userId);
            return $item;
        });
